...
void mesh_skinned::apply_skinning(skeleton_geometry const& skeleton) //Sj = skeleton
{
    int const N_vertex = size_vertex();
    ASSERT_CPE(N_vertex==int(vertices_original_data.size()),"Incorrect size");
    std::cout << skeleton[0].orientation << std::endl ;
    for(int k_vertex=0 ; k_vertex<N_vertex ; ++k_vertex)
    {
        ...
        {
            int id_joint = skinning_info[k_joint].joint_id;
            p_final += skinning_info[k_joint].weight*(skeleton[id_joint].orientation*p0+skeleton[id_joint].position);
        }
