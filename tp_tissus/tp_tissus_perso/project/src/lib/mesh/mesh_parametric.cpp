...
void mesh_parametric::set_plane_xy_unit(int const size_u_param,int const size_v_param, float offset_x, float offset_y)
{
    ASSERT_CPE(size_u_param>0 && size_v_param>0 , "Problem of mesh size");
    size_u_data = size_u_param;
    size_v_data = size_v_param;
    ...
    for(int kv=0 ; kv<size_v_param ; ++kv)
    {
        float const v = static_cast<float>(kv)/(size_v_param-1) + offset_y;
        for(int ku=0 ; ku<size_u_param ; ++ku)
        {
            float const u = static_cast<float>(ku)/(size_u_param-1) + offset_x;
